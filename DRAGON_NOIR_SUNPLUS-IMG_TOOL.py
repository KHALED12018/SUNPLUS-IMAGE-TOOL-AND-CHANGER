...
class SunplusEngine:
    ...
    def extract(self):
        ...
        while True:
            ...
            blocks.append(p)
            pos = p + 3
        self.log(f"LZMA blocks found: {len(blocks)}")
        idx = 0
        for b in blocks:
            try:
                dec = lzma.decompress(self.data[b:])
                self._scan(dec, b, idx)
                idx += 1
            except:
                continue
        self.log(f"Images detected: {len(self.images)}")
        return self.images

    def _scan(self, data, offset, block):
        pos = 0
        while True:
    ...
    def replace(self, idx, raw_new_data):
        ...
        self.log(f"Replaced image {idx} inside payload boundaries.")
        return True
    ...
    def fix_crc(self):
        try:
            ...
            self.log(f"CRC successfully fixed: {hex(crc)}")
        except Exception as e:
            self.log(f"CRC adjustment bypassed: {str(e)}")

class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("DRAGON_NOIR_SUNPLUS-IMAGE CHANG")
    ...
    def open(self):
        ...
        for i, im in enumerate(imgs):
            self.listbox.insert("end", f" BLOCK ADDR: {i:03d} | Dimensions: {im.width}x{im.height} Px")
    ...
    def extract_original(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("JPEG Package", "*.jpg")],
            initialfile=f"dump_index_{self.selected}_{target.width}x{target.height}.jpg"
        )
        if not path:
            return
        try:
            ...
            self.log(f"Extracted copy committed to disk: {path}")
            messagebox.showinfo("Extraction Done", "Original copy mirrored and saved successfully.")
        except Exception as ex:
            self.log(f"Extraction Pipeline Failure: {str(ex)}")

    def load_new_image(self):
        if self.selected is None:
            return
        path = filedialog.askopenfilename(filetypes=[("Valid Graphics Images", "*.jpg *.jpeg *.png *.bmp")])
        if not path:
            return
        try:
            ...
            self.log(f"Failed loading graphic file: {str(ex)}")
    ...
    def merge_image(self):
        ...
        if self.engine.replace(self.selected, final_data):
            messagebox.showinfo("Buffer Synced", "Memory replacement blocks matched and compressed back to active container safely.")

    def save_file(self):
        if not self.engine:
            return
        self.engine.fix_crc()
        path = filedialog.asksaveasfilename(defaultextension=".bin", filetypes=[("Raw ROM Binaries", "*.bin"), ("All Files", "*.*")])
        if not path:
            return
        with open(path, "wb") as f:
            f.write(self.engine.data)
        self.log(f"Firmware package compiled cleanly to: {path}")
        messagebox.showinfo("Process Finished", "Operational code compiled and written down safely.")
